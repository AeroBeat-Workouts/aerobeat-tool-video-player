...
func test_readme_describes_video_player_facade_truth() -> void:
...
func test_testbed_contains_real_ogv_proving_surface() -> void:
	assert_true(ResourceLoader.exists(TESTBED_SCENE_PATH), "video player proving scene should exist")
	assert_true(FileAccess.file_exists(ProjectSettings.globalize_path(SAMPLE_VIDEO_PATH)), "real .ogv sample should exist inside the repo-local testbed surface")
