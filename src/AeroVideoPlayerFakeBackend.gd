...
var _rate: float = 1.0
var _cover_mode: String = COVER_MODE_CONTAIN
...
func load(source: Dictionary) -> Dictionary:
	...
	_cover_mode = _normalize_cover_mode(_source.get("cover_mode", COVER_MODE_CONTAIN))
	_audio_level = _normalize_audio_level(_source.get("audio_level", 1.0))
	_source["cover_mode"] = _cover_mode
...
func set_cover_mode(cover_mode: String) -> Dictionary:
	...
	if not _source.is_empty():
		_source["cover_mode"] = _cover_mode
	return AeroVideoPlaybackContract.ok({"cover_mode": _cover_mode})
...
func _normalize_cover_mode(value: Variant) -> String:
	...
	return normalized if COVER_MODES.has(normalized) else COVER_MODE_CONTAIN
